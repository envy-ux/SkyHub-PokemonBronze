--[[
    ...
    One-Shot Kill • Force 100% Catch / Master Ball
    GET/POST + Battle remote aware (no classic _p required)
    Executor: Solora (Android / emulator friendly)
    UI open: tap "Sky Hub" ShowText button (no physical K key needed)
]]
...
if not okRay or not Rayfield then
    print("[SkyHub] FAIL: rayfield load — " .. tostring(errRay))
    return
end
print("[SkyHub] rayfield ok")
...
-- ══════════════════════════════════════
-- WINDOW (Solora / mobile: ShowText = "Sky Hub")
-- ══════════════════════════════════════

print("[SkyHub] creating window")
local Window = Rayfield:CreateWindow({
    Name = "Sky Hub • Pokémon Bronze v3",
    Icon = 0,
    LoadingTitle = "Sky Hub",
    LoadingSubtitle = "Pokémon Bronze Premium v3 • Solora",
    ShowText = "Sky Hub",
    Theme = "Default",
    ToggleUIKeybind = "K",
...
print("[SkyHub] window ok")

local MainTab = Window:CreateTab("Main")
local BattleTab = Window:CreateTab("Battle")
local ItemsTab = Window:CreateTab("Items")
local CatchTab = Window:CreateTab("Catch")
local TeleportTab = Window:CreateTab("Teleport")
local VisualTab = Window:CreateTab("Visual")
local MiscTab = Window:CreateTab("Misc")
print("[SkyHub] tabs ok")
    ...
    Callback = function()
        SafeCall(function()
            if _p and _p.Battle and _p.Battle.currentBattle then
                ...
                if b.opponent and type(b.opponent) == "table" then
                    for _, mon in pairs(b.opponent) do
...
local function TryGive(itemName, amount)
    ...
    SafeCall(function()
        if getgenv and getgenv().giveItem then
...
ItemsTab:CreateButton({
    Name = "Ultra Ball x100",
    Callback = function() TryGive("Ultra Ball", 100) end,
})

ItemsTab:CreateSection("Healing / Other")

ItemsTab:CreateButton({
    Name = "Max Potion x50",
    Callback = function() TryGive("Max Potion", 50) end,
})

ItemsTab:CreateButton({
    Name = "Full Restore x50",
    Callback = function() TryGive("Full Restore", 50) end,
})

ItemsTab:CreateButton({
    Name = "Revive x30",
    Callback = function() TryGive("Revive", 30) end,
})

ItemsTab:CreateButton({
    Name = "Max Revive x20",
    Callback = function() TryGive("Max Revive", 20) end,
})

ItemsTab:CreateParagraph({
    Title = "Item note",
    Content = "This PlaceId has no classic _p bag. Tries GET/POST + bag tables. Server may ignore client-side give — test and report.",
})
...
CatchTab:CreateSection("Force Catch")

CatchTab:CreateToggle({
    Name = "Force 100% Catch",
    CurrentValue = false,
    Flag = "ForceCatch",
    Callback = function(v)
        State.ForceCatch = v
...
CatchTab:CreateButton({
    Name = "Force Master Ball Now",
    Callback = function()
        SafeCall(function()
            if _p and _p.Battle and _p.Battle.currentBattle then
                if _p.Battle.currentBattle.throwBall then
...
CatchTab:CreateParagraph({
    Title = "Catch tip",
    Content = "Force 100% + Always Master Ball work best together. One-Shot Kill is under Battle tab. All paths fire GET/POST + Battle remotes for this PlaceId. Solora: tap ShowText 'Sky Hub' to open UI.",
})
...
MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        Notify("Misc", "FPS Boost applied")
    end,
})

MiscTab:CreateButton({
    Name = "Destroy Sky Hub UI",
    Callback = function()
        pcall(function()
            Rayfield:Destroy()
        end)
...
MiscTab:CreateParagraph({
    Title = "Solora + PlaceId",
    Content = "Executor: Solora. PlaceId: 122591147665527. No classic _p. Open UI via ShowText button 'Sky Hub'. Read Solora console for [SkyHub] lines.",
})

MiscTab:CreateParagraph({
    Title = "Feature status",
    Content = "Client: WalkSpeed/Noclip/ESP work. One-Shot / Force Catch / Items need battle test — report enemy dies? ball catches? nothing?",
})
    ...
    while task.wait(0.3) do
        if State.UnlimitedHP or State.GodMode then
            SafeCall(function()
                ...
                if _p and _p.Battle and _p.Battle.currentBattle then
                    local b = _p.Battle.currentBattle
                    if b.side and type(b.side) == "table" then
                        for _, mon in pairs(b.side) do
                            if type(mon) == "table" and mon.maxHP then
                                mon.hp = mon.maxHP
                                mon.HP = mon.maxHP
                                mon.currentHP = mon.maxHP
...
Notify("Sky Hub v3 Loaded", "Solora ready. Tap 'Sky Hub' to open. One-Shot • Force Catch ready.")
print("[SkyHub] FULLY LOADED")
print("[Sky Hub v3] Pokémon Bronze Premium loaded — Solora • PlaceId aware")
